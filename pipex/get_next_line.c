...
static char	*ft_new_lines(char *read_lines)
{
	int		i;
	int		j;
	char	*new_lines;
...
static char	*ft_lines(char *read_lines)
{
	char	*lines;
	int		i;
...
static char	*ft_read_lines(int fd, char *read_lines)
{
	char	*buffer;
	int		r;
	...
	while (!ft_strchr(read_lines, '\n') && r != 0)
	{
		...
		buffer[r] = '\0';
		read_lines = ft_strjoin_get_next_line(read_lines, buffer);
	}
	free (buffer);
	return (read_lines);
}
